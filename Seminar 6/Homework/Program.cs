...
int[,] FillBorder(int rows, int columns)
{
    ...
    {
        // сразу красим верхнюю и нижнюю границу
        border_matrix[0, i] = 1;
        border_matrix[rows - 1, i] = 1;
        for (int j = 1; j < rows - 1; j++) // исключаем верхнюю и нижнюю границы в условии
        {
            border_matrix[j, 0] = 1; // докрасили левую и правую
            border_matrix[j, columns - 1] = 1;  
            // по умолчанию уже 0 подставлены в пустой integer массив, но попробуем все таки "заполнить" потом, как просят в условии.
            //border_matrix[j,i] = 0; Здесь снова 0ми станут верхняя и нижняя границы, поэтому без другой записи циклов или if  мне не обойтись.
            //Также возникли проблемы с печатью, почему-то работает при рядах и столбцах больше 5.
